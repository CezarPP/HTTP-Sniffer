...
def parse_ethernet_header(raw_data: bytes) -> (str, str, int, bytes):
    # Ethernet header:
    ...
    destination_mac: str = ":".join(f"{byte:02x}" for byte in destination_mac)
    source_mac: str = ":".join(f"{byte:02x}" for byte in source_mac)
